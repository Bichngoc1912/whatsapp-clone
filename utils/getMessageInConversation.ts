...
export const transformMessage = (message: QueryDocumentSnapshot<DocumentData>) => ({
  ...
  sent_at: message.data().sent_at ? convertFirestoreTimstampToString(message.data().sent_at as Timestamp) : null,
}) as IMessage;

export const convertFirestoreTimstampToString = (timestamp: Timestamp)  => {
